...
function Dashboard() {
  ...
  const tabBtnCls = (name: string) => `px-5 py-2 rounded-[20px] border-[1.5px] border-[#ff6b9d] font-semibold text-[13px] cursor-pointer w-auto mt-0 transition-all
    ${isTab(name) ? 'bg-gradient-to-br from-[#ff6b9d] to-[#ff8fb8] text-white border-transparent' : 'bg-white text-[#ff6b9d] hover:bg-gradient-to-br hover:from-[#ff6b9d] hover:to-[#ff8fb8] hover:text-white hover:border-transparent'}`;

  const inputCls = "w-full px-3 py-2.5 border border-[#ffd6e0] rounded-[6px] font-[Poppins,sans-serif] text-[14px] outline-none focus:border-[#ff6b8a] box-border";
  const labelCls = "block text-[13px] font-medium mb-1 text-[#444]";
  const thCls    = "px-4 py-3 text-left font-semibold text-[13px]";
  const tdCls    = "px-4 py-3 text-[#444] align-middle break-words";
  ...
  return (
    <div className="m-0 font-[Poppins,sans-serif] bg-[#fff5f9] text-[#1f1f1f]">
      ...
      <div className="max-w-[1100px] mx-auto px-5 py-8">
        ...
        {/* STATS */}
        <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Store Statistics</h2>
        <div className="flex flex-wrap justify-center gap-4 mb-9">
          {[
            ...
            <div key={lbl} onClick={() => showTab(tab, true)}
              className="flex-1 min-w-[150px] max-w-[200px] bg-white rounded-[16px] px-[18px] py-[22px] text-center shadow-[0_4px_18px_rgba(255,107,157,0.1)] border border-[#ffe0ee] border-t-4 border-t-[#ff6b9d] cursor-pointer transition-all hover:-translate-y-1 hover:shadow-[0_12px_28px_rgba(255,107,157,0.28)]">
              <div className="text-[28px] mb-2">{icon}</div>
              <p className={`text-[26px] font-bold m-0 ${alert ? 'text-[#e8527f]' : 'text-[#ff6b9d]'}`}>{num}</p>
              <p className="text-[12px] text-[#888] mt-1 font-medium">{lbl}</p>
            </div>
          ))}
        </div>

        {/* ACTIONS */}
        <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Dashboard Actions</h2>
        <div className="flex flex-wrap justify-center gap-4 mb-9">
          {[
            ...
            <div key={tab + btn} className="flex-1 min-w-[200px] max-w-[260px] bg-white rounded-[16px] px-5 py-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] flex flex-col transition-all hover:-translate-y-1 hover:shadow-[0_10px_26px_rgba(255,107,157,0.22)]">
              <h3 className="text-[15px] font-semibold m-0 mb-2 text-[#1f1f1f]">{title}</h3>
              <p className="text-[12px] text-[#888] m-0 mb-3.5 leading-relaxed flex-1">{desc}</p>
        ...
        <div id="section-stock" className={isTab('stock') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">All Inventory Records</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
        ...
        <div id="section-orders" className={isTab('orders') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">New Orders <span className="inline-block bg-[#e8527f] text-white text-[11px] font-bold px-2 py-0.5 rounded-[20px] ml-1.5">{newOrders.length}</span></h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            {newOrders.length > 0 && <p className="text-[13px] text-[#888] mb-3.5">Orders placed by customers not yet reviewed.</p>}
            <div className="w-full overflow-x-auto mt-5">
              <table className="w-full border-collapse rounded-[15px] overflow-hidden text-[14px]">
                <thead className="bg-[#ff6b9d] text-white">
                ...
                <tbody>
                  {newOrders.length === 0
                    ...
                    : newOrders.map((o, i) => (
                      <tr key={i} className={`${i % 2 === 0 ? 'bg-[#fff5f7]' : 'bg-[#ffe5ec]'} hover:bg-[#ffc2d1]`}>
          ...
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">All Customer Orders</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            <div className="w-full overflow-x-auto mt-5">
              <table className="w-full border-collapse rounded-[15px] overflow-hidden text-[14px]">
                <thead className="bg-[#ff6b9d] text-white">
        ...
        <div id="section-chart" className={isTab('chart') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Stock Graphical View</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            <p className="text-[13px] text-[#888] mb-3.5">Bar chart showing total stock quantity per category.</p>
            <canvas ref={stockChartRef} id="stockChart" width={600} height={280} className="max-w-full rounded-[10px]"></canvas>
          </div>
          <h2 className="text-[20px] font-bold mb-4 mt-8 text-[#1f1f1f]">Orders Overview</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            <p className="text-[13px] text-[#888] mb-3.5">Bar chart showing total orders received, new (unseen) and seen orders.</p>
        ...
        <div id="section-categories" className={isTab('categories') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Product Categories</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            <div className="w-full overflow-x-auto mt-5">
              <table className="w-full border-collapse rounded-[15px] overflow-hidden text-[14px]">
                <thead className="bg-[#ff6b9d] text-white">
        ...
        <div id="section-add" className={isTab('add') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Add New Product</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
        ...
        <div id="section-log" className={isTab('log') ? 'block' : 'hidden'}>
          <h2 className="text-[20px] font-bold mb-4 text-[#1f1f1f]">Recent Activity Log</h2>
          <div className="bg-white rounded-[16px] p-6 shadow-[0_4px_18px_rgba(255,107,157,0.08)] border border-[#ffe0ee] mb-6">
            <p className="text-[13px] text-[#888] mb-3.5">Latest actions performed on the inventory.</p>
            <ul className="list-none p-0 m-0">
              {log.length === 0
                ? <li className="flex items-center gap-3 py-2.5 text-[13px] text-[#555]"><span className="text-[18px] min-w-[28px] text-center">📋</span> No activity yet.</li>
                : log.map((l, i) => (
                  <li key={i} className={`flex items-center gap-3 py-2.5 text-[13px] text-[#555] ${i < log.length - 1 ? 'border-b border-[#fff0f5]' : ''}`}>
                    <span className="text-[18px] min-w-[28px] text-center">{l.icon}</span>
                    {l.msg}
                    <span className="text-[11px] text-[#aaa] ml-auto whitespace-nowrap">{l.time}</span>
                  </li>
                ))
              }
            </ul>
