    ...
    <div className="min-h-screen font-[Poppins,sans-serif] bg-[#fff5f9] dark:bg-[#1a0a10] text-[#1f1f1f] dark:text-[#f5e6ea]">
      ...
      <div className="max-w-[1100px] mx-auto px-5 py-8">
        ...
        <div className="flex items-center gap-3 mb-5 justify-center">
          <h2 className="text-[24px] font-semibold m-0 text-[#1f1f1f] dark:text-[#f5e6ea]">Your Cart</h2>
                ...
                {Object.keys(grouped).length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center p-5">
                      <div className="flex flex-col items-center py-10 gap-3">
                        ...
                        <p className="text-[#888] dark:text-[#9a7a85] text-[15px] m-0">Your cart is empty</p>
                  ...
                  Object.values(grouped).map((item, i) => (
                    <tr key={i} className={`${i % 2 === 0 ? 'bg-[#fff5f7] dark:bg-[#2a1020]' : 'bg-[#ffe5ec] dark:bg-[#3a1828]'} hover:bg-[#ffc2d1] dark:hover:bg-[#4a2030] cursor-pointer`}>
                      <td className="px-4 py-3 text-[#444] dark:text-[#d4b8c0] align-middle"><strong>{item.name}</strong></td>
                      <td className="px-4 py-3 text-[#444] dark:text-[#d4b8c0] align-middle">Rs. {item.price}</td>
                      <td className="px-4 py-3 text-[#444] dark:text-[#d4b8c0] align-middle">
                        <div className="flex items-center gap-2">
                          <button onClick={() => changeQty(item.name, item.price, -1)} className="w-7 h-7 p-0 rounded-full text-[16px] font-bold m-0 flex items-center justify-center bg-[#ffd6e8] dark:bg-[#4a2030] text-[#ff6b9d] border border-[#ffd6e0] dark:border-[#4a2030] cursor-pointer transition-colors hover:bg-[#ff6b9d] hover:text-white">−</button>
                          <span className="font-semibold text-[15px] min-w-[20px] text-center text-[#1f1f1f] dark:text-[#f5e6ea]">{item.qty}</span>
                          <button onClick={() => changeQty(item.name, item.price, 1)} className="w-7 h-7 p-0 rounded-full text-[16px] font-bold m-0 flex items-center justify-center bg-[#ffd6e8] dark:bg-[#4a2030] text-[#ff6b9d] border border-[#ffd6e0] dark:border-[#4a2030] cursor-pointer transition-colors hover:bg-[#ff6b9d] hover:text-white">+</button>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-[#444] dark:text-[#d4b8c0] align-middle">Rs. {item.subtotal}</td>
                      <td className="px-4 py-3 text-[#444] dark:text-[#d4b8c0] align-middle">
                        <button onClick={() => removeByName(item.name)} className="bg-transparent border-none text-[18px] cursor-pointer px-2 py-1 rounded-md w-auto m-0 transition-colors hover:bg-[#ffe0ee] dark:hover:bg-[#3a1828]">🗑️</button>
                      </td>
                    </tr>
                  ))
                )}
        ...
        {/* SUMMARY */}
        <div className="max-w-[500px] mx-auto mb-10 bg-white dark:bg-[#2a1020] rounded-[20px] px-8 py-7 shadow-[0_8px_30px_rgba(255,107,157,0.15)] border border-[#ffe0ee] dark:border-[#4a2030]">
          <div className="flex justify-between text-[15px] text-[#444] dark:text-[#d4b8c0] mb-3"><span>Subtotal</span><span>Rs. {subtotal}</span></div>
          <div className="flex justify-between text-[15px] text-[#444] dark:text-[#d4b8c0] mb-3"><span>Delivery</span><span>{delivery === 0 && subtotal > 0 ? 'FREE 🚚' : `Rs. ${delivery}`}</span></div>
          <div className="border-t border-[#ffe0ee] dark:border-[#4a2030] my-4"></div>
          <div className="flex justify-between text-[18px] font-bold text-[#1f1f1f] dark:text-[#f5e6ea] mb-1.5"><span>Estimated Total</span><span>Rs. {subtotal + delivery}</span></div>
          <p className="text-[11px] text-[#888] dark:text-[#9a7a85] mb-5">Taxes and discounts calculated at checkout</p>
          {subtotal >= 10000 && (
            <div className="bg-gradient-to-br from-[#fff0f5] to-[#ffe0ee] dark:from-[#3a1828] dark:to-[#2a1020] border border-[#ffd6e0] dark:border-[#4a2030] rounded-[10px] px-3.5 py-2.5 text-[13px] font-semibold text-[#ff6b9d] text-center mb-2.5">🎁 Yay! A free gift has been added to your order!</div>
          )}
          {subtotal > 0 && subtotal < 10000 && (
            <p className="text-[12px] text-[#ff6b9d] font-medium text-center mb-2 bg-[#fff0f5] dark:bg-[#3a1828] px-3 py-2 rounded-[10px] border border-dashed border-[#ff8fb8]">Spend Rs. {10000 - subtotal} more for a FREE gift! 🎁</p>
          )}
          ...
          <button onClick={clearCart} className="w-full mt-1.5 px-4 py-2.5 bg-white dark:bg-[#2a1020] text-[#ff6b9d] border-2 border-[#ff6b9d] font-semibold rounded-[20px] cursor-pointer text-[13px] transition-all hover:bg-gradient-to-br hover:from-[#ff6b9d] hover:to-[#ff8fb8] hover:text-white hover:border-transparent">Clear Cart</button>
        </div>
      </div>
