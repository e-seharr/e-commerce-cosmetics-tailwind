  ...
  return (
    <div className="m-0 font-[Poppins,sans-serif] bg-[#fff5f9] text-[#1f1f1f]">
      ...
      <div className="flex justify-between items-center flex-wrap gap-3 max-w-[1200px] mx-auto mt-6 mb-2 px-5">
        <h2 className="text-[24px] font-semibold m-0 text-[#1f1f1f]">All Products 💄</h2>
        <div className="flex items-center gap-3 flex-wrap">
          <input
            ...
            className="w-[220px] px-[18px] py-[11px] rounded-[30px] border-[1.5px] border-[#f0d0d8] bg-white text-[13px] shadow-[0_2px_10px_rgba(255,107,157,0.08)] outline-none focus:border-[#ff6b9d]"
          />
          <select
            value={sort}
            onChange={e => setSort(e.target.value)}
            className="w-auto min-w-[160px] px-5 py-[11px] pr-10 rounded-[30px] text-[14px] font-medium border-[1.5px] border-[#f0d0d8] bg-white text-[#1f1f1f] shadow-[0_2px_10px_rgba(255,107,157,0.08)] appearance-none cursor-pointer outline-none"
          >
      ...
      <div className="flex flex-wrap gap-2 mx-3 my-2">
        {activeTag && (
          <div className="flex items-center gap-1.5 bg-white border border-[#ffd6e0] rounded-[20px] px-3 py-1 text-[12px] text-[#444]">
            {activeTag}
      ...
      <div className="flex gap-6 max-w-[1200px] mx-auto px-5 pb-10 items-start max-md:flex-col">
        ...
        {/* SIDEBAR */}
        <div className="w-[220px] min-w-[200px] bg-white rounded-[15px] shadow-[0_4px_18px_rgba(255,107,157,0.1)] border border-[#ffe0ee] py-4 shrink-0 max-md:w-full">
          <h3 className="px-4 pb-3 text-[14px] font-bold uppercase tracking-[0.5px] text-[#1f1f1f] border-b border-[#ffe0ee] mb-0">Filter By</h3>

          {/* Category */}
          <div className="border-b border-[#ffe0ee]">
            <div onClick={() => toggleAcc('category')} className={`flex justify-between items-center px-4 py-3 cursor-pointer text-[14px] font-medium transition-colors select-none hover:bg-[#fff5f9] ${openAcc === 'category' ? 'text-[#ff6b9d] bg-[#fff5f9]' : 'text-[#1f1f1f]'}`}>
            ...
            <div className={accBodyClass('category')}>
              {[['all','All Products'],['makeup','💄 Makeup'],['skincare','🧴 Skincare'],['haircare','💆 Haircare'],['nailcare','💅 Nailcare']].map(([val, label]) => (
                <a key={val} href="#" onClick={e => { e.preventDefault(); handleSetCat(val); }}
                  className={`block px-6 py-2 text-[13px] no-underline transition-colors rounded-md hover:text-[#ff6b9d] hover:bg-[#fff5f9] ${cat === val && !sub && !saleOnly ? 'text-[#ff6b9d] font-semibold' : 'text-[#444]'}`}>{label}</a>
              ))}
            </div>
          </div>
          ...
          {/* Makeup Type */}
          <div className="border-b border-[#ffe0ee]">
            <div onClick={() => toggleAcc('makeup')} className={`flex justify-between items-center px-4 py-3 cursor-pointer text-[14px] font-medium transition-colors select-none hover:bg-[#fff5f9] ${openAcc === 'makeup' ? 'text-[#ff6b9d] bg-[#fff5f9]' : 'text-[#1f1f1f]'}`}>
            ...
            <div className={accBodyClass('makeup')}>
              {[['lips','👄 Lips'],['eyes','👁️ Eyes'],['face','✨ Face'],['nails','💅 Nails']].map(([val, label]) => (
                <a key={val} href="#" onClick={e => { e.preventDefault(); handleSetSub(val); }}
                  className={`block px-6 py-2 text-[13px] no-underline transition-colors rounded-md hover:text-[#ff6b9d] hover:bg-[#fff5f9] ${sub === val ? 'text-[#ff6b9d] font-semibold' : 'text-[#444]'}`}>{label}</a>
              ))}
            </div>
          </div>
          ...
          {/* Price Range */}
          <div className="border-b border-[#ffe0ee]">
            <div onClick={() => toggleAcc('price')} className={`flex justify-between items-center px-4 py-3 cursor-pointer text-[14px] font-medium transition-colors select-none hover:bg-[#fff5f9] ${openAcc === 'price' ? 'text-[#ff6b9d] bg-[#fff5f9]' : 'text-[#1f1f1f]'}`}>
            ...
            <div className={accBodyClass('price')}>
              <div className="px-4 pb-3.5 pt-2.5 flex flex-col gap-2">
                <input type="number" placeholder="Min Rs." value={minP} onChange={e => setMinP(e.target.value)} className="w-full px-3 py-2.5 border border-[#ffd6e0] rounded-[6px] text-[14px] font-[Poppins,sans-serif] outline-none focus:border-[#ff6b8a] box-border" />
                <input type="number" placeholder="Max Rs." value={maxP} onChange={e => setMaxP(e.target.value)} className="w-full px-3 py-2.5 border border-[#ffd6e0] rounded-[6px] text-[14px] font-[Poppins,sans-serif] outline-none focus:border-[#ff6b8a] box-border" />
          ...
          {/* Discount */}
          <div className="border-b border-[#ffe0ee]">
            <div onClick={() => toggleAcc('discount')} className={`flex justify-between items-center px-4 py-3 cursor-pointer text-[14px] font-medium transition-colors select-none hover:bg-[#fff5f9] ${openAcc === 'discount' ? 'text-[#ff6b9d] bg-[#fff5f9]' : 'text-[#1f1f1f]'}`}>
            ...
            <div className={accBodyClass('discount')}>
              <a href="#" onClick={e => { e.preventDefault(); setSaleOnly(true); setActiveTag('On Sale'); }}
                className={`block px-6 py-2 text-[13px] no-underline transition-colors rounded-md hover:text-[#ff6b9d] hover:bg-[#fff5f9] ${saleOnly ? 'text-[#ff6b9d] font-semibold' : 'text-[#444]'}`}>🏷️ On Sale Only</a>
              <a href="#" onClick={e => { e.preventDefault(); handleSetCat('all'); }}
                className={`block px-6 py-2 text-[13px] no-underline transition-colors rounded-md hover:text-[#ff6b9d] hover:bg-[#fff5f9] ${!saleOnly && cat === 'all' ? 'text-[#ff6b9d] font-semibold' : 'text-[#444]'}`}>All Products</a>
            </div>
          </div>

          <div className="px-4 pb-3.5 pt-2.5">
            <button onClick={clearAllFilters} className="w-full mt-0 px-4 py-2.5 bg-white text-[#ff6b9d] border-2 border-[#ff6b9d] font-semibold rounded-[20px] cursor-pointer text-[13px] transition-all hover:bg-gradient-to-br hover:from-[#ff6b9d] hover:to-[#ff8fb8] hover:text-white hover:border-transparent">Clear All Filters</button>
          </div>
        </div>
        ...
        <div className="flex-1">
          <div className="flex flex-wrap gap-4 justify-start max-md:justify-center">
            {filtered.map((p, i) => {
              ...
              return (
                <div key={i} className="flex flex-col bg-white rounded-[14px] w-[280px] overflow-hidden shadow-[0_4px_18px_rgba(255,107,157,0.1)] border border-[#ffe0ee] cursor-pointer transition-all relative hover:-translate-y-1 hover:shadow-[0_12px_28px_rgba(255,107,157,0.3)] hover:border-[#ff6b9d]">
                  ...
                  <img src={p.img} alt={p.name} className="w-full h-[240px] object-contain bg-[#fff8fb] p-3 box-border" />
                  <div className="px-4 pt-3.5 pb-4 flex flex-col gap-1.5">
                    <h3 className="text-[15px] font-semibold m-0 text-[#1f1f1f]">{p.name}</h3>
                    <div className="min-h-[44px] flex flex-col justify-end">
                      {p.oldPrice && <p className="line-through text-[#888] text-[12px] m-0">Rs. {p.oldPrice}</p>}
                    ...
                    <button
                      ...
                      className="mt-1.5 px-4 py-2.5 bg-white text-[#ff6b9d] border-2 border-[#ff6b9d] font-semibold rounded-[20px] cursor-pointer text-[13px] w-full transition-all hover:bg-gradient-to-br hover:from-[#ff6b9d] hover:to-[#ff8fb8] hover:text-white hover:border-transparent"
                    >Buy Now</button>
                  </div>
                </div>
              );
